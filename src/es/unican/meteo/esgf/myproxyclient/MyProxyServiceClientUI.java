...
public class MyProxyServiceClientUI extends JFrame {
	...
	private static String[] nodes = { "esgf-index1.ceda.ac.uk", "pcmdi9.llnl.gov",
		...
		"esgdata.gfdl.noaa.gov", "dev.esg.anl.gov", "esg.bnu.edu.cn", 
		"adm07.cmcc.it", "euclipse1.dkrz.de", "esgf.nccs.nasa.gov", 
		"esg-datanode.jpl.nasa.gov", "esg2.nci.org.au", "esg01.nersc.gov",
		"esg.ccs.ornl.gov" };
			...
			public void actionPerformed(ActionEvent paramActionEvent) {
				try {
					...
					if (user != null && password != null && !user.equals("")
						...
						if (idpCombo.getSelectedItem().equals("<< Custom OpenID URL >>")) {
							credentialsProvider.initialize(user, password);
						} else {
							String completeOpenID = "https://"
									+ idpCombo.getSelectedItem()
									+ "/esgf-idp/openid/" + user;
							credentialsProvider.initialize(completeOpenID, password);
						}
						
						//retrieve credentials
						System.out.println("Using "+credentialsProvider.getMyProxyLib()+" lib...");								
	...
	private JPanel generateWriteOptionPanel() {
		...
		ItemListener writeOptionsListener= new ItemListener() {
			...
			public void itemStateChanged(ItemEvent paramItemEvent) {
				JCheckBox source = (JCheckBox) paramItemEvent.getItemSelectable();
				if (source == chkPem) {
					credentialsProvider.setWritePem(source.isSelected());
				} else if (source == chkJKS) {
					credentialsProvider.setWriteJKSKeystore(source.isSelected());
				} else if (source == chkJCEKS) {
					credentialsProvider.setWriteJCEKSKeystore(source.isSelected());
				} else if (source == chkTruststore) {
					credentialsProvider.setWriteTruststore(source.isSelected());
				} else if (source == chkCerts) {
					credentialsProvider.setWriteTrustRootsCerts(source.isSelected());
	...
	private JPanel generateIdPanel() {
        ...
        idFolOptsPanel.setBorder(BorderFactory.createTitledBorder(
                "Select folder:"));
        
        //Options of MyProxy Lib in idpPanel
        JPanel idLibOptsPanel = new JPanel(new GridLayout(2,1));
        final JRadioButton rbMyProxyLogon = new JRadioButton("use MyProxyLogon lib (v-1.4.6)");
        rbMyProxyLogon.setSelected(true);
        final JRadioButton rbMyProxy206= new JRadioButton("use MyProxy lib (v-2.0.6)");
        final ButtonGroup myProxyGroup = new ButtonGroup(); //radio button group
        ...
        idLibOptsPanel.setBorder(BorderFactory.createTitledBorder(
                "Select lib:"));
        ...
        ActionListener idOptionsListener= new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent paramActionEvent) {
				JRadioButton source = (JRadioButton) paramActionEvent.getSource();
        		if (source == rbEsgFol) {
        			...
        			credentialsProvider.setMyProxyLib(ESGFCredentialsProvider.Lib.MYPROXYLOGON);
        		}else if(source== rbMyProxy206){
        			credentialsProvider.setMyProxyLib(ESGFCredentialsProvider.Lib.MYPROXYV206);
        		}
			}
		};
